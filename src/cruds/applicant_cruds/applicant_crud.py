...
from src.models.model import Applicant, Education, User, Resume
...
class ApplicantCrud(BaseCrud):
    ...
    async def get_by_user_id(self, db: AsyncSession, user_id: int) -> Applicant | None:
        stmt = select(Applicant).join(User, User.applicant_id == Applicant.id).where(User.id == user_id)
    ...
    async def get_by_user_id_with_details(self, db: AsyncSession, user_id: int) -> Applicant | None:
        stmt = (
            ...
            .options(
                ...
                selectinload(Applicant.educations).selectinload(Education.institution),
            )
        )
        result = await db.execute(stmt)
        return result.scalar_one_or_none()
